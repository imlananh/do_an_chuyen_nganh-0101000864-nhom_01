...
#define SOIL_VERY_WET 1000    // Đất rất ẩm 
#define SOIL_GOOD 2000        // Đất đủ ẩm
#define SOIL_DRY 2500         // Đất khô, cần tưới
#define SOIL_VERY_DRY 3500    // Đất rất khô, cần tưới ngay
...
void doamdat() {
    value = analogRead(34); 
    int soilMoisture = 4095 - value; // Đảo ngược giá trị
    ...
    if(toggleState == LOW) { // Chế độ Auto
        if(reading == HIGH) { // Nếu có nước trong bồn
            if(soilMoisture >= SOIL_VERY_DRY) {
                ...
                Serial.println(" - Dat rat kho, bat dau tuoi ngay!");
            }
            else if(soilMoisture >= SOIL_DRY) {
                digitalWrite(relay, HIGH);
                digitalWrite(led1, HIGH);
                ERa.virtualWrite(V20, HIGH); 
                Serial.println(" - Dat kho, can tuoi");
            }
        ...
        if(soilMoisture <= SOIL_VERY_WET) {
            Serial.println(" - Dat rat am");
        } else if(soilMoisture <= SOIL_GOOD) {
            Serial.println(" - Dat du am");
        } else if(soilMoisture <= SOIL_DRY) {
            Serial.println(" - Dat hoi kho, can tuoi");
        } else {
            Serial.println(" - Dat rat kho, can tuoi ngay");
        }
    }
    delay(1000);
}
...
void printLocalTime() {
   ...
   if (toggleState == LOW) { // Chế độ Auto
       ...
       // Thêm điều kiện kiểm tra đất rất khô - tưới ngay 
       if(reading == HIGH && soilMoisture >= SOIL_VERY_DRY) {
           digitalWrite(relay, HIGH);
           digitalWrite(led1, HIGH);
           ERa.virtualWrite(V20, HIGH);
           Serial.println("Dat rat kho, tuoi ngay lap tuc!");
       }
       ...
       if ((timeinfo.tm_year + 1900) == e && (timeinfo.tm_mon + 1) == d && timeinfo.tm_mday == c 
           ...
           if(reading == HIGH && soilMoisture >= SOIL_DRY) {
               digitalWrite(relay, HIGH);
               digitalWrite(led1, HIGH);
               ERa.virtualWrite(V20, HIGH);
               Serial.print("Bat dau tuoi - Do am dat: ");
               Serial.println(soilMoisture);
           }
           else if(reading == HIGH && soilMoisture <= SOIL_VERY_WET) {
               digitalWrite(relay, LOW);
               digitalWrite(led1, LOW);
               ERa.virtualWrite(V20, LOW);
               Serial.println("Dat qua am, khong can tuoi");
           }
       }
