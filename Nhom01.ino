...
void printLocalTime() {
   ...
   if (toggleState == LOW) { // Chế độ Auto
       ...
       if ((timeinfo.tm_year + 1900) == e && (timeinfo.tm_mon + 1) == d && timeinfo.tm_mday == c 
           ...
           if(reading == HIGH && soilMoisture <= SOIL_VERY_DRY) {    // Đất đủ ẩm thì dừng
               digitalWrite(relay, HIGH);
               digitalWrite(led1, HIGH);
               ERa.virtualWrite(V20, HIGH);
           ...
           else if(reading == HIGH && soilMoisture >= SOIL_VERY_WET) {    // Đất đủ ẩm thì dừng
               digitalWrite(relay, LOW);
               digitalWrite(led1, LOW);
               ERa.virtualWrite(V20, LOW);
               Serial.println("Dat du am, khong can tuoi");
           }
       }
